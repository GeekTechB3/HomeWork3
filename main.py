...
async def mailing(message : types.Message):
    await message.answer('Почта билдирүүңүздү киргизиңиз: ')
...
async def mailing(message : types.Message, state : FSMContext):
    ...
    try:
        await message.answer("Бюллетень башталды")
        ...
        await message.answer("Ката кийинчерээк дагы аракет кылып келген")
        await state.finish()
...
async def download_audio(message: types.Message, state : FSMContext):
    try:
        ...
        try:
            ...
            await message.answer("Ката кетти, бираздансон кайтала")
        await state.finish()
    except:
        await message.answer("Неверная ссылка на видео")
        await state.finish()
